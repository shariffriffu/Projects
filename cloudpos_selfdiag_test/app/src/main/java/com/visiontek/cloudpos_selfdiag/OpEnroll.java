...
public abstract class OpEnroll extends Thread implements PtGuiStateCallback
{
    ...
    private PtInputBir enroll() throws PtException
    {
        ...
        try 
        {
            ...
            System.out.println("-------------"+s.getDevice().toString());



            onDisplayMessage("=========enroll completed===========");
            printText("enroll completed");

        }
        catch (PtException e)
        {
            onDisplayMessage("Enrollment failed - " + e.getMessage());
            printText("Enrollment failed");
            throw e;
        }
                ...
                for(int i=0; i<fingerList.length; i++)
                {
                    ...
                    if((fingerData != null) && (fingerData.length >= 1))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                onDisplayMessage("Finger already enrolled");
                                printText(String.format("Finger already enrolled"));
                                return false;
                            }
                            else{
                                onDisplayMessage("Finger enrolled");
                                printText(String.format("Finger enrolled"));
                            }
                        }
                    }
                }
